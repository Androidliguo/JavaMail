...
public class MySendMailThread extends Thread {
	...
	public void run() {
		...
		try {
			...
			// 邮件信息封装
			// 1发件人
			msg.setFrom(new InternetAddress("szu_lg@sina.com"));
			// 2收件人
			...
			sbd.append("<font color='red'><a href='http://localhost:8080/myMailWeb/ActiveServlet?acode="
					+ user.getAcode() + "' target='_blank'");
			sbd.append(">立即激活</a></font><br/>");
			sbd.append("或者点击下面链接:<br/>");
			sbd.append("http://localhost:8080/myMailWeb/ActiveServlet?acode="
					+ user.getAcode() + "<br/>");
